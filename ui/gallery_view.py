...
class GalleryView(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.duplicates_switch.setToolTip("Afficher uniquement les doublons / similaires")
        self.duplicates_switch.toggled.connect(self._on_duplicates_toggled)
    ...
    def load_media(self, source_dir: str) -> None:
        ...
        # Décoche le mode doublons sans relancer d'analyse.
        self.duplicates_switch.blockSignals(True)
        self.duplicates_switch.setChecked(False)
        self.duplicates_switch.blockSignals(False)
    ...
    def _reset_filters(self) -> None:
        """Réinitialise tri/filtre/recherche (sans déclencher de réaffichage)."""
        self._sort_key, self._filter, self._search = "name", "all", ""
        for widget in (self._sort_combo, self._filter_combo, self._search_edit):
    ...
    def _display(self, media_list: list[scanner.MediaFile]) -> None:
        ...
        # Regroupe par section en respectant l'ordre des sections du scanner.
        by_section: dict[str, list[scanner.MediaFile]] = {}
        for media in media_list:
            by_section.setdefault(media.section, []).append(media)
        for name in sorted(by_section, key=scanner.section_key):
            self._create_section(name)
    ...
    def _create_section(self, name: str) -> _Section:
        ...
        # Position ordonnée parmi les sections existantes (racine d'abord).
        keys = [scanner.section_key(n) for n in self._section_order]
        pos = bisect.bisect_left(keys, scanner.section_key(name))
    ...
    def _add_path(self, path: str) -> None:
        ...
        name = self._section_for_path(path)
        ...
        try:
            ...
            size = 0
        self._media_by_path[path] = scanner.MediaFile(
            path=path, section=name, is_video=is_video, size=size
        )
    ...
    def select_all_but_first_per_group(self) -> None:
        ...
        if self._dup_index is None:
            return
        to_select: list[str] = []
        for members in self._dup_index.groups.values():
            ordered = sorted(members, key=lambda p: os.path.basename(p).lower())
    ...
    def _compute_view(self) -> list[scanner.MediaFile]:
        """Applique filtre + recherche + tri à la base courante."""
        ...
        if self._sort_key == "size":
            media.sort(key=lambda m: m.size)
        elif self._sort_key == "date":
            media.sort(key=lambda m: (self._date_of(m.path) is None,
                                      self._date_of(m.path) or 0))
        else:  # name
            media.sort(key=lambda m: os.path.basename(m.path).lower())
        return media
    ...
    def _on_duplicates_toggled(self, checked: bool) -> None:
        if checked:
            if not self._media_by_path:
                self.duplicates_switch.setChecked(False)
                return
    ...
    def _on_duplicates_ready(self, index) -> None:
        # L'utilisateur a pu décocher entre-temps : ignorer le résultat obsolète.
        if not self.duplicates_switch.isChecked():
            return
        self._dup_index = index
        self._dup_color = dict(index.color_of)
        self._active_group = None
        n_ident = sum(1 for c in index.color_of.values() if c == duplicates.COLOR_IDENTICAL)
        n_sim = sum(1 for c in index.color_of.values() if c == duplicates.COLOR_SIMILAR)
        view = self._compute_view()
        if not view:
            self._display([])
            self._placeholder.setText("Aucun doublon ni similaire détecté.")
            self._placeholder.show()
        else:
            self._display(view)
        self.status.emit(
            f"Doublons : {n_ident} identiques (rouge), {n_sim} similaires (orange). "
